...
class ProjectNameAnonymized(BaseAnonymizer):
    ...
    def _load_words(self):
        ...
        if not path:
            raise ValueError("Не указан параметр 'words_source' для project_name")
    ...
    def _generate(self, 
        ...
        max_w = self.params.get("max_words", 5)
        ...
        if preserve and value:
            ...
            best_name = None
            best_diff = float("inf")
            # Детерминированный перебор – используем seed для выбора слов
            for n in range(min_w, max_w + 1):
                rng = random.Random(seed + n)
                # Выбираем n уникальных слов детерминированно
